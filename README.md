...
2. Если файл больше лимита Bot API (~50 MB) — бот предлагает **сжать до 50 MB** (пропорции сохраняются), а для YouTube ещё и перекачать в меньшем качестве, что обычно быстрее сжатия
3. Перед сжатием бот пишет примерное время, а во время — проценты; если ролик не укладывается в бюджет времени, честно об этом сообщает
4. Случайный анекдот категории Б
...
Лимит Bot API ≈ **50 MB** на файл: облачный `api.telegram.org` на файле в 60 MB
отвечает `413 Request Entity Too Large` (проверено). Больше — бот предложит
сжатие, а не молча откажет.
...
Первый id в этом файле — владелец: только ему доступна `/stats`.  
Статистика скачиваний — в `bot_stats.json` (тоже локально).  
