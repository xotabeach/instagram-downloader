...
- сжатие до 50 MB — только по кнопке, FFmpeg `veryfast`, 1 поток, таймаут 15 минут
- разрешение при сжатии подбирается под целевой битрейт (1080p при ~1.5 Mbps не кодируется)
- systemd: MemoryHigh 220 MB, MemoryMax 300 MB, 1 CPU
- при 403 YouTube перебирает клиенты плеера `android_vr` → `web_embedded` → `tv`

Лимит памяти важен для скорости: если FFmpeg выходит за `MemoryHigh`, cgroup
начинает его тротлить, и кодирование 1080p падает до ~0.05x реального времени.
Поэтому x264 запускается с коротким lookahead, а кадр уменьшается заранее.
