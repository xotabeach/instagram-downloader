...
- сжатие до 50 MB — только по кнопке, FFmpeg `superfast`, 1 поток, 30 fps
- разрешение подбирается под целевой битрейт (1080p при ~1.5 Mbps не кодируется)
- таймаут сжатия — вдвое от расчётной длительности, но не больше 25 минут
- systemd: MemoryHigh 220 MB, MemoryMax 300 MB, 1 CPU
- при 403 YouTube перебирает клиенты плеера `android_vr` → `web_embedded` → `tv`

Про скорость сжатия на этом VPS (один общий Broadwell-ядро, ~50% steal time):

- если FFmpeg выходит за `MemoryHigh`, cgroup тротлит его, и 1080p кодируется
  со скоростью ~0.05x реального времени — отсюда короткий x264 lookahead
- декодирование исходника 1080p60 само по себе идёт ~0.8x реального времени,
  поэтому трёхминутный ролик быстрее пяти-семи минут не сожмётся
- для YouTube выгоднее перекачать ролик в меньшем качестве, чем перекодировать;
  бот предлагает это кнопкой рядом со «Сжать»
